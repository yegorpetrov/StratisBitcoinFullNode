...
namespace FxCoin.CryptoPool.DbWallet.Broadcasting
{
    public class BroadcasterBehavior : NetworkPeerBehavior
    {
        ...
        {
            return new BroadcasterBehavior(broadcasterManager, logger);
        }
        ...
        private void ProcessInvPayload(InvPayload invPayload)
        {
            ...
            {
                TransactionBroadcastEntry txEntry = broadcasterManager.GetTransaction(inv.Hash);
                if (txEntry != null)
                {
                    broadcasterManager.AddOrUpdate(txEntry.Transaction, State.Propagated);
                }
            }
        }
        ...
        protected async Task ProcessGetDataPayloadAsync(INetworkPeer peer, GetDataPayload getDataPayload)
        {
            ...
            foreach (InventoryVector inv in getDataPayload.Inventory.Where(x => x.Type == InventoryType.MSG_TX))
            {
                ...
                if (txEntry != null && txEntry.State != State.CantBroadcast)
                {
                    ...
                    {
                        broadcasterManager.AddOrUpdate(txEntry.Transaction, State.Broadcasted);
                    }
                }
            }
        }
        ...
        {
            AttachedPeer.MessageReceived.Register(OnMessageReceivedAsync);
        }
        ...
        {
            AttachedPeer.MessageReceived.Unregister(OnMessageReceivedAsync);
        }
    }
}
